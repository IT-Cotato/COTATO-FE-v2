...
export const MyPageSubmittedApplicationsContainer = () => {
  ...
  return (
    <div className='flex items-center justify-center bg-white py-23'>
      <div className='flex min-h-79.25 max-w-480 min-w-360 flex-col items-center justify-center gap-8 rounded-[10px] bg-neutral-50 px-40'>
        <h1 className='text-h4 flex w-full items-start text-neutral-800'>
          지원 현황
        </h1>
        <div className='w-275 overflow-hidden'>
          <table className='w-full text-center'>
            <thead className='text-h5 rounded-[10px] bg-white text-neutral-500'>
              <tr>
                <th className='px-6 py-4 font-semibold'>기수</th>
                <th className='px-6 py-4 font-semibold'>지원 파트</th>
            ...
            <tbody>
              {applications?.map((item) => (
                <tr key={item.applicationId}>
                  ...
                  <td className='text-body-l px-6 py-5 text-neutral-800'>
                    {PART_LABEL_MAP[item.part as keyof typeof PART_LABEL_MAP] ||
                      item.part}
                  </td>
