...
export const HomeBannerContainer = () => {
  ...
  return (
    <section
      ...
      <div
        className={`absolute inset-0 z-10 bg-black/40 backdrop-blur-[6px] transition-opacity duration-500 ${isOverlayHidden ? 'opacity-0' : 'opacity-100'} lg:opacity-0 lg:group-hover:bg-black/60 lg:group-hover:opacity-100 lg:group-hover:backdrop-blur-[6px]`}
        aria-hidden='true'
      />

      <div
        className={`absolute top-1/3 left-1/2 z-20 flex -translate-x-1/2 -translate-y-2/3 flex-col items-center transition-opacity duration-300 ${isOverlayHidden ? 'opacity-0' : 'opacity-100'} lg:opacity-0 lg:group-hover:opacity-100`}>
        <CotatoLogo />
      </div>

      <div
        className={`absolute bottom-60 left-1/2 z-20 -translate-x-1/2 transition-all duration-300 ${isOverlayHidden ? 'opacity-0' : 'opacity-100'} lg:opacity-0 lg:group-hover:opacity-100`}>
        <button
