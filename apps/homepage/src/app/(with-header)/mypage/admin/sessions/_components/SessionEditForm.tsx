...
export const SessionEditForm = ({form, onChange}: SessionEditFormProps) => {
  ...
  return (
    <>
      ...
      />
      <div className='flex w-full flex-col gap-3.75 rounded-[10px] bg-white p-2.5 md:px-8.5 md:py-9'>
        <div className='flex flex-col gap-1'>
          <p className='text-h5 font-semibold text-neutral-400'>
            {form.generation}
          </p>
          <input
            type='text'
            ...
            placeholder='9회차 세션'
            className={`text-h5 lg:text-h3 font-bold ${INPUT_BASE} px-4 py-1.25 lg:py-3`}
          />
        </div>
        <div className='py-0'>
          <div className='h-px w-full shrink-0 bg-neutral-200' />
        </div>
        <div className='grid grid-cols-1 items-start gap-x-12 gap-y-4.5 lg:grid-cols-[auto_1fr] lg:items-start lg:gap-y-2'>
          <div className='flex flex-col gap-2.5 lg:contents'>
            <p className='text-h5 font-semibold text-neutral-600'>세션 장소</p>
            <div className='flex items-center gap-1 lg:gap-5'>
              <div
                onClick={() => setIsLocationModalOpen(true)}
                className={`flex cursor-pointer items-center ${INPUT_BASE} h-8 w-full gap-2.5 px-1.5 py-1.25 md:w-27.5`}>
              ...
              <input
                ...
                placeholder='상세 주소'
                className={`text-body-m ${INPUT_BASE} h-8 w-full px-1.5 py-1.25 placeholder:text-neutral-400 md:hidden`}
              />
              <input
                type='text'
                value={form.detailAddress}
                onChange={(e) => set('detailAddress', e.target.value)}
                placeholder='상세 주소 (예: 세미나실 1)'
                className={`text-body-m ${INPUT_BASE} hidden h-8 w-27.5 px-1.5 py-1.25 placeholder:text-neutral-400 md:block`}
              />
              <button
                type='button'
                aria-label='장소 삭제'
          ...
          <div className='flex items-center gap-9.75 lg:contents lg:gap-2.5'>
            <p className='text-h5 font-semibold text-neutral-600'>대면 여부</p>
          ...
          <div className='flex flex-col gap-2.5 lg:contents'>
            <p className='text-h5 font-semibold text-neutral-600'>
              출석 인정 시간
            </p>
            <div className='flex gap-1 lg:gap-5'>
              <div
                className={`flex items-center ${INPUT_BASE} h-8 w-full gap-2.5 px-1.5 py-1.25 md:w-27.5`}>
          ...
          <div className='flex flex-col gap-2.5 lg:contents'>
            <p className='text-h5 font-semibold text-neutral-600'>세션 내용</p>
            <textarea
              value={form.description}
              onChange={(e) => set('description', e.target.value)}
              placeholder='세션 내용을 입력해주세요.'
              rows={2}
              className={`text-body-m ${INPUT_BASE} resize-none px-2 py-1.5`}
            />
          </div>
        </div>
      </div>
