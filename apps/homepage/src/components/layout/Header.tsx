    ...
    <header className='z-header sticky top-0 flex h-22 w-full min-w-360 items-center justify-between bg-black pr-26.25 pl-6.25'>
      ...
      <nav className='flex items-center gap-5'>
        {NAV_ITEMS.map(({label, href, external}) => {
          ...
          const baseClasses =
            'text-h5 px-4.25 py-6 transition-colors duration-300 hover:text-white';
        ...
        {/** 로그인 안 되어 있을 때 */}
        <Link href={ROUTES.ONBOARDING} className='text-primary text-h5'>
          LOGIN
        </Link>
        ...
        <Link
          href={ROUTES.MYPAGE}
          type='button'
          className='text-h5 flex flex-row items-center justify-center gap-1.25 px-4.25 py-6 text-white'>
          <SmallLogo className='h-4 w-4 text-white' /> 김감자
        </Link>

        {/** 출석 활성화 시간일 때 */}
        <Link
          href=''
          className='border-primary text-h5 bg-primary/30 rounded-[10px] border px-6 py-1.5 text-white'>
          출석하기
        </Link>
      </nav>
    </header>
